...
int8 CRunningScript::ProcessCommands300To399(int32 command)
{
	...
	case COMMAND_LOAD_SPECIAL_CHARACTER:
	{
		...
		debug("TODO VCS NEED TEST COMMAND_LOAD_SPECIAL_CHARACTER\n");
		...
		CStreaming::RequestSpecialChar(GET_INTEGER_PARAM(0) - 1, strbuff, STREAMFLAGS_DEPENDENCY | (m_bIsMissionScript ? STREAMFLAGS_SCRIPTOWNED : STREAMFLAGS_AMBIENT_SCRIPT_OWNED));
		return 0;
	}
	...
	case COMMAND_IS_PLAYER_IN_REMOTE_MODE:
	{
		...
		// vcs
		GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0));
		UpdateCompareFlag(pPlayerInfo->IsPlayerInRemoteMode());
		return 0;
	}
	...
	case COMMAND_RESTART_CRITICAL_MISSION:
	{
		...
		// vcs bug? get ped from pool by GET_INTEGER_PARAM(0)
		//GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0));
		CWorld::Players[CWorld::PlayerInFocus].PlayerFailedCriticalMission();
		return 0;
	}
	case todo__comm_363: //016B=2,set_coll_obj_wander_dir 
	{
	...
	case todo__comm_374: //0176=1,set_coll_obj_leave_car %1d%
	{
	...
	case todo__comm_377: // 0179=2,set_coll_obj_destroy_object %1d% object %2d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_377");
		return 0;
	}
	case todo__comm_378: // 017A=2,set_coll_obj_destroy_car %1d% car %2d%
	{
	...
	case todo__comm_381: // 017D=3,set_coll_obj_follow_char_in_formation %1d% char %2d% formation_type %3d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_381");
		return 0;
	}
	case todo__comm_382: // 017E=1,set_coll_obj_steal_any_car %1d%
	{
	...
	/*case COMMAND_SET_COLL_OBJ_LEAVE_CAR:
		...
		return 0;*/
	case todo__comm_384: // 0180=2,set_coll_obj_flee_car %1d% vehicle %2d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_384");
		return 0;
	}
	case todo__comm_385: // 0181=2,set_coll_obj_goto_char_on_foot_walking %1d% char %2d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_385");
		return 0;
	}
	case todo__comm_386: // 0182=2,set_coll_obj_aim_gun_at_char %1d% char %2d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_386");
		return 0;
	}
	case todo__comm_387: // 0183=3,set_coll_obj_sprint_to_coord %1d% point %2d% %3d%
	{
	...
	case todo__comm_390: // 0186=1,set_coll_obj_take_taxi %1d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_390");
		return 0;
	}
	case todo__comm_391: // 0187=6,set_coll_obj_unknown_6464 %1d% point %2d% %3d% %4d% radius %5d% value %6d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_391");
		return 0;
	}
	case todo__comm_392: // 0188=1,clear_coll_follow_path %1d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_392");
		return 0;
	}
	case todo__comm_393: // 0189=2,  unknown_coll_command_6433 flag %1d% char %2d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_393");
		return 0;
	}
	case todo__comm_394: // 018A=2,  unknown_coll_check_7453 %1d% char %2d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_394");
		return 0;
	}
	case todo__comm_395: // 018B=1,unknown_coll_command_8521 char %1d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_395");
		return 0;
	}
	case todo__comm_396: // 018C=4,add_peds_in_area_to_coll %1d% point %2d% %3d% %4d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_396");
		return 0;
	}
	case todo__comm_397: // 018D=2,  add_peds_in_vehicle_to_coll %1d% vehicle %2d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_397");
		return 0;
	}
	case todo__comm_398: // 018E=1,unknown_coll_command_6423 %1d%
	{
		script_assert(false && "TODO_OR_CHECK_todo__comm_398");
		return 0;
	}
	case todo__comm_399: // 018F=1,  unknown_coll_check_7353 %1d%
	{
...
int8 CRunningScript::ProcessCommands400To499(int32 command)
{
	...
	case COMMAND_SWITCH_WIDESCREEN:
		...
		if (GET_INTEGER_PARAM(0) != 0)
			...
			// TODO(LCS): unknown field // mazahaka VCS unknown byte_6F4FD5
	...
	case COMMAND_GET_RANDOM_CHAR_IN_ZONE:
	{
		...
		//if(nZone != -1) { m_nIp += KEY_LENGTH_IN_SCRIPT; }

		// vcs
		const char *label = GetKeyFromScript(&m_nIp);
		int nZone = CTheZones::FindZoneByLabelAndReturnIndex((char*)label, ZONE_DEFAULT);
	...
	case COMMAND_LOAD_CUTSCENE:
	{
		...
		// unknown call FUN_29df68(name) on PS2 - not on PSP // lcs
		//m_nIp += KEY_LENGTH_IN_SCRIPT;

		// vcs
		//char name[16];
		const char *str = GetKeyFromScript(&m_nIp);
		//strncpy(name, str, KEY_LENGTH_IN_SCRIPT);


		CColStore::RemoveAllCollision();
		CCutsceneMgr::LoadCutsceneData(str);
		return 0;
	}
	...
	case COMMAND_LOAD_SPECIAL_MODEL: // 01D1=2,load_special_model %1o% %2s%
	{
		// lcs
		//CollectParameters(&m_nIp, 1);
		//char name[KEY_LENGTH_IN_SCRIPT];
		...
		// vcs
		CollectParameters(&m_nIp, 1);
		int mi = GET_INTEGER_PARAM(0); // in vcs ScriptParams used for mi, but simplify is store mi before

		char strbuff[16]; // mb KEY_LENGTH_IN_SCRIPT
		const char *str = GetKeyFromScript(&m_nIp);
		strncpy(strbuff, str, KEY_LENGTH_IN_SCRIPT);
		for(int i = 0; i < KEY_LENGTH_IN_SCRIPT; i++) { strbuff[i] = tolower(strbuff[i]); }
		CStreaming::RequestSpecialModel(mi, strbuff, STREAMFLAGS_DEPENDENCY | (m_bIsMissionScript ? STREAMFLAGS_SCRIPTOWNED : STREAMFLAGS_AMBIENT_SCRIPT_OWNED));
	...
	case COMMAND_REGISTER_MISSION_PASSED:
	{
		...
		//vcs
		//script_assert(false && "VCS CHECK: COMMAND_REGISTER_MISSION_PASSED IS CORRECT SET szmname");
		const char *str = GetKeyFromScript(&m_nIp);
		script_assert(str);
		//wchar *text = TheText.Get(str); // mb ITBEG or move names 2 gxt
		//wchar *text = GetTextByKeyFromScript(&m_nIp); // from COMMAND_PRINT_SOON // VCS NOTE: in code CText but strncpy is key
		strncpy(CStats::LastMissionPassedName, str, KEY_LENGTH_IN_SCRIPT);
	...
	case COMMAND_IS_CHAR_IN_PLAYERS_GROUP:
	{
		CollectParameters(&m_nIp, 2);
		CPed* pPed = CPools::GetPedPool()->GetAt(GET_INTEGER_PARAM(0));
		//CPed* pLeader = CWorld::Players[GET_INTEGER_PARAM(1)].m_pPed; // lcs
		CPed *pLeader = CPools::GetPedPool()->GetAt(GET_INTEGER_PARAM(1)); // vcs
		script_assert(pPed);
	...
	case COMMAND_SET_ZONE_GROUP:
	{
		...
		CollectParameters(&m_nIp, 2);
		int zone_id = CTheZones::FindZoneByLabelAndReturnIndex((char*)label, ZONE_INFO);
		if (zone_id < 0) {
		...
		while (zone_id >= 0) {
			...
			zone_id = CTheZones::FindNextZoneByLabelAndReturnIndex((char*)label, ZONE_INFO);
		}
		return 0;
	}
