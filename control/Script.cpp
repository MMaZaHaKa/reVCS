...
// vcs todo inline or macro
const char* CRunningScript::GetKeyFromScript(uint32* pIp) // lcs, vcs hardcoded or inlined
{ // uint32* pIp not need, we can use this->m_nIp but need recode handlers so todo
...
int8 CRunningScript::ProcessOneCommand()
{
	...
	// like REGISTER_COMMAND_HANDLER(COMMAND_IS_INT_VAR_GREATER_OR_EQUAL_TO_NUMBER, IsGreaterEqual<int32>);
