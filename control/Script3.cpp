...
int8 CRunningScript::ProcessCommands500To599(int32 command)
{
	...
	{
		CollectParameters(&m_nIp, 2);
		//CPlayerInfo* pPlayer = &CWorld::Players[GET_INTEGER_PARAM(0)]; // lcs
		//pPlayer->m_bInfiniteSprint = (GET_INTEGER_PARAM(1) != 0);

		// vcs
		GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0));
		pPlayerInfo->m_bInfiniteSprint = (GET_INTEGER_PARAM(1) != 0);
		return 0;
	}
	case COMMAND_SET_PLAYER_FAST_RELOAD:
	{
		...
		// vcs
		GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0));
		pPlayerInfo->m_bFastReload = (GET_INTEGER_PARAM(1) != 0);
		return 0;
	}
	...
	case COMMAND_REMOVE_CHAR_ELEGANTLY:
	{
		CollectParameters(&m_nIp, 1);
		CPed* pPed = CPools::GetPedPool()->GetAt(GET_INTEGER_PARAM(0));
		if (pPed && !pPed->IsPlayer() && pPed->CharCreatedBy == MISSION_CHAR) // vcs
		{
		...
		if (!pPed->IsPlayer() && m_bIsMissionScript) // vcs
	...
	case COMMAND_UNDRESS_CHAR:
	{
		...
		// vcs
		char strbuff[16]; // ? KEY_LENGTH_IN_SCRIPT
		const char *str = GetKeyFromScript(&m_nIp);
		strncpy(strbuff, str, KEY_LENGTH_IN_SCRIPT);
		for(int i = 0; i < KEY_LENGTH_IN_SCRIPT; i++) { strbuff[i] = tolower(strbuff[i]); }
		pPed->Undress(strbuff);
		// vcs
#ifdef FIX_BUGS // from COMMAND_LOAD_ALL_MODELS_NOW
		CTimer::Suspend();
#else
		CTimer::Stop();
#endif
	...
	case COMMAND_ADD_ARMOUR_TO_CHAR:
	{
		...
		script_assert(pPed);
		// vcs wtf?
		//if(pPed->IsPlayer()) // todo?
		//	((CPlayerPed *)pPed)->GetPlayerInfoForThisPlayerPed();
	...
	{
		CollectParameters(&m_nIp, 2);
		//CPlayerPed* pPed = CWorld::Players[GET_INTEGER_PARAM(0)].m_pPed; // lcs

		// vcs
		GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0));
		CPlayerPed *pPed = pPlayerInfo->m_pPed;
	...
	case COMMAND_DISPLAY_ONSCREEN_TIMER_WITH_STRING:
	{
		...
		// vcs
		CollectParameters(&m_nIp, 1); // type
		bool bGoingDown = (GET_INTEGER_PARAM(0) != 0);
		const char *label = GetKeyFromScript(&m_nIp);

		CUserDisplay::OnscnTimer.AddClock(var, (char*)label, bGoingDown);
		return 0;
	}
	case COMMAND_DISPLAY_ONSCREEN_COUNTER_WITH_STRING:
	{
		...
		// vcs
		CollectParameters(&m_nIp, 1); // type
		uint32 type = GET_INTEGER_PARAM(0); // uint16
		const char *label = GetKeyFromScript(&m_nIp);

		CUserDisplay::OnscnTimer.AddCounter(var, type, (char*)label, 0, -1, nil, 0);
		return 0;
	}
	...
	{
		CollectParameters(&m_nIp, 1);
		// vcs todo some mi code miobj < 474
	...
	{
		CollectParameters(&m_nIp, 1);
		//CVehicle *pVehicle = CPools::GetVehiclePool()->GetAt(GET_INTEGER_PARAM(0)); // vcs why ?
	...
	case COMMAND_LOAD_MISSION_AUDIO:
	{
		...
		// vcs
		CollectParameters(&m_nIp, 1);
		script_assert(false && "VCS TODO CHECK COMMAND_LOAD_MISSION_AUDIO"); // lcs code is tolower and slot -1 !!!!
		uint16 slot = GET_INTEGER_PARAM(0);

		//char strbuff[16]; // comment version is lcs logic: slot-1 and str tolower TODO OR TEST!! in vcs not -1 and tolower
		const char *label = GetKeyFromScript(&m_nIp);
		//strncpy(strbuff, label, KEY_LENGTH_IN_SCRIPT);
		//for(int i = 0; i < KEY_LENGTH_IN_SCRIPT; i++) { strbuff[i] = tolower(strbuff[i]); }
		//DMAudio.PreloadMissionAudio(slot - 1, strbuff);
		DMAudio.PreloadMissionAudio(slot, label);
		return 0;
	}
...
int8 CRunningScript::ProcessCommands600To699(int32 command)
{
	...
	{
		CollectParameters(&m_nIp, 1);
		//UpdateCompareFlag(DMAudio.GetMissionAudioLoadingStatus(GET_INTEGER_PARAM(0) - 1) == 1); // lcs
		UpdateCompareFlag(DMAudio.GetMissionAudioLoadingStatus(GET_INTEGER_PARAM(0)) == 1); // vcs ^ 1
		return 0;
	}
	case COMMAND_PLAY_MISSION_AUDIO:
	{
		...
		// vcs
		CollectParameters(&m_nIp, 2);
		DMAudio.PlayLoadedMissionAudio(GET_INTEGER_PARAM(0), GET_INTEGER_PARAM(1) != 0); // vcs
	...
	{
		CollectParameters(&m_nIp, 1);
		//UpdateCompareFlag(DMAudio.IsMissionAudioSampleFinished(GET_INTEGER_PARAM(0) - 1)); // lcs
		UpdateCompareFlag(DMAudio.IsMissionAudioSampleFinished(GET_INTEGER_PARAM(0))); // vcs
		return 0;
	}
	case COMMAND_GET_CLOSEST_CAR_NODE_WITH_HEADING:
	{
		...
		if (pos.z <= MAP_Z_LOW_LIMIT)
			pos.z = CWorld::FindGroundZForCoord(pos.x, pos.y);
		int node = ThePaths.FindNodeClosestToCoors(pos, 0, 999999.9f, true, true); // vcs todo
		// int node = ThePaths.FindNodeClosestToCoors(&v10, 0i64, 1i64, 1i64, 0i64, 0i64, 0i64, 800.0);
	...
	case COMMAND_SET_MISSION_AUDIO_POSITION:
	{
		CollectParameters(&m_nIp, 4);
		CVector pos = GET_VECTOR_PARAM(1);
		//DMAudio.SetMissionAudioLocation(GET_INTEGER_PARAM(0) - 1, pos.x, pos.y, pos.z); // lcs
		DMAudio.SetMissionAudioLocation(GET_INTEGER_PARAM(0), pos.x, pos.y, pos.z); // vcs
		return 0;
	}
	...
	{
		CollectParameters(&m_nIp, 1);
		//CPlayerPed* pPlayerPed = CWorld::Players[GET_INTEGER_PARAM(0)].m_pPed; // lcs

		// vcs
		//GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0));
		//CPlayerPed *pPlayerPed = pPlayerInfo->m_pPed;
		// or vcs ida style
		CPed *pPlayerPed = CPools::GetPedPool()->GetAt(GET_INTEGER_PARAM(0));
		script_assert(pPlayerPed);
	...
	{
		CollectParameters(&m_nIp, 1);
		//CPlayerInfo* pPlayerInfo = &CWorld::Players[GET_INTEGER_PARAM(0)]; // lcs
	...
	case COMMAND_CLEAR_MISSION_AUDIO:
		CollectParameters(&m_nIp, 1);
		//DMAudio.ClearMissionAudio(GET_INTEGER_PARAM(0) - 1); // lcs
		DMAudio.ClearMissionAudio(GET_INTEGER_PARAM(0));
		return 0;
	...
	{
		CollectParameters(&m_nIp, 2);
		//CWorld::Players[GET_INTEGER_PARAM(0)].m_bGetOutOfJailFree = !!GET_INTEGER_PARAM(1); // lcs

		// vcs
		GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0));
		pPlayerInfo->m_bGetOutOfJailFree = !!GET_INTEGER_PARAM(1);
		return 0;
	}
	case COMMAND_SET_FREE_HEALTH_CARE:
	{
		CollectParameters(&m_nIp, 2);
		//CWorld::Players[GET_INTEGER_PARAM(0)].m_bGetOutOfHospitalFree = !!GET_INTEGER_PARAM(1); // lcs

		// vcs
		GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0));
		pPlayerInfo->m_bGetOutOfHospitalFree = !!GET_INTEGER_PARAM(1);
		return 0;
	}
	...
	case COMMAND_GET_AMMO_IN_CHAR_WEAPON:
	{
		CollectParameters(&m_nIp, 2);
		CPed* pPed = CPools::GetPedPool()->GetAt(GET_INTEGER_PARAM(0));
		//debug("RECHECK COMM VCS COMMAND_GET_AMMO_IN_CHAR_WEAPON\n");
		script_assert(pPed);
	...
	case COMMAND_SET_TOTAL_NUMBER_OF_MISSIONS:
		CollectParameters(&m_nIp, 1);
		debug("VCS: COMM COMMAND_SET_TOTAL_NUMBER_OF_MISSIONS not write in orig game\n");
		CStats::SetTotalNumberMissions(CGame::germanGame ? GET_INTEGER_PARAM(0) - 2 : GET_INTEGER_PARAM(0)); // not in vcs
		return 0;
	...
	case COMMAND_GET_DEBUG_CAMERA_COORDINATES:
		...
		StoreParameters(&m_nIp, 3);
		return 0;
	case COMMAND_IS_CHAR_TARGETTING_CHAR: // aka COMMAND_IS_PLAYER_TARGETTING_CHAR
	{
		CollectParameters(&m_nIp, 2);
		//CPlayerPed* pPed = CWorld::Players[GET_INTEGER_PARAM(0)].m_pPed; // lcs
		CPed* pPed = CPools::GetPedPool()->GetAt(GET_INTEGER_PARAM(0));
		...
		//debug("TODO VCS: NEED TEST COMMAND_IS_CHAR_TARGETTING_CHAR\n");
#ifdef GTA_PC // vcs need?
	...
	case COMMAND_IS_CHAR_TARGETTING_OBJECT: // aka COMMAND_IS_PLAYER_TARGETTING_OBJECT
	{
		CollectParameters(&m_nIp, 2);
		//CPlayerPed* pPed = CWorld::Players[GET_INTEGER_PARAM(0)].m_pPed; // lcs
		CPed *pPed = CPools::GetPedPool()->GetAt(GET_INTEGER_PARAM(0));
	...
	case COMMAND_TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME:
	{
		...
		//script_assert(false && "test this");
		CRunningScript* pScript = CTheScripts::pActiveScripts;
		// vcs if(pScript)
		while (pScript) {
			CRunningScript* pNext = pScript->next;
			if (strcmp(pScript->m_abScriptName, strbuff) == 0) {
				// vcs
				if(pScript->m_bMissionFlag) {
					...
					script_assert(pPlayerPed);
					CPlayerInfo* pPlayerInfo = ((CPlayerPed *)pPlayerPed)->GetPlayerInfoForThisPlayerPed();
	...
	case COMMAND_DETACH_CHAR_FROM_CAR:
	{
		...
		if (pPed && pPed->m_attachedTo) {
			...
			if(pPed->IsPlayer())
			{
				 // *(_DWORD *)(pPed + 3248) = 0; // todo
				 // *(_DWORD *)(pPed + 3252) = 0; // todo
				script_assert(false && "VCS TODO THIS COMM: COMMAND_DETACH_CHAR_FROM_CAR");
			}

		}
		return 0;
	}
	case COMMAND_SET_PLAYER_HEADING: // 02B8=2,unknown_command_vcs_02B9 ped %1d% angle %2d%
	{
		CollectParameters(&m_nIp, 2);
		script_assert(false && "TODO_OR_CHECK_todo__comm_696 COMMAND_SET_PLAYER_HEADING");
		CPed *pPed = CPools::GetPedPool()->GetAt(GET_INTEGER_PARAM(0));
		script_assert(pPed);
		if(pPed->bInVehicle) { return 0; }
		// vcs recheck fields!!
...
int8 CRunningScript::ProcessCommands700To799(int32 command)
{
	...
	case COMMAND_GET_REMOTE_CONTROLLED_CAR:
	{
		...
		// vcs
		GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0));
		CVehicle *pVehicle = pPlayerInfo->m_pRemoteVehicle;
	...
	case COMMAND_REMOVE_ALL_CHAR_WEAPONS:
	{
		...
		CPed* pPed = CPools::GetPedPool()->GetAt(GET_INTEGER_PARAM(0));
		script_assert(pPed);
		debug("VCS TODO CODE comm:COMMAND_REMOVE_ALL_CHAR_WEAPONS\n");
		/*if (!pPed->IsPlayer()) // vcs
		{
			pPed->ClearFollowPath();
			pPed->RestartNonPartialAnims();
			RpAnimBlendClumpRemoveAllAssociations(pPed->GetClump());
			// vcs todo *(_DWORD *)(pPed + 776) = 0;
			CAnimManager::AddAnimation(pPed->GetClump(), AssocGroupId::ASSOCGRP_STD, ANIM_STD_STARTWALK);
		}*/
		pPed->ClearWeapons();
		// vcs todo  *(_QWORD *)(pPed + 456) &= ~0x100ui64;
		return 0;
	}
	...
	{
		CollectParameters(&m_nIp, 1);
		//CWorld::Players[GET_INTEGER_PARAM(0)].m_nHavocLevel = 0; // lcs

		// vcs
		GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0));
		pPlayerInfo->m_nHavocLevel = 0;
		return 0;
	}
	case COMMAND_GET_HAVOC_CAUSED_BY_PLAYER:
	{
		CollectParameters(&m_nIp, 1);
		//SET_INTEGER_PARAM(0, CWorld::Players[GET_INTEGER_PARAM(0)].m_nHavocLevel); // lcs

		// vcs
		GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0));
		SET_INTEGER_PARAM(0, pPlayerInfo->m_nHavocLevel);
		StoreParameters(&m_nIp, 1);
		return 0;
	}
	...
	case COMMAND_IS_CHAR_IN_FLYING_VEHICLE: // COMMAND_IS_PLAYER_IN_FLYING_VEHICLE
	{
		CollectParameters(&m_nIp, 1);
	...
	case COMMAND_FREEZE_CHAR_POSITION:
	{
		...
		// VCS TODO: some code with zone and if else
		return 0;
	}
	...
	case COMMAND_SET_ZONE_CIVILIAN_CAR_INFO:
	{
		...
		int i;
		script_assert(false && "VCS TODO: COMMAND_SET_ZONE_CIVILIAN_CAR_INFO 14 args");
		...
		// vcs
		//char strbuff[16]; // TEST THIS!! IP
		const char *str = GetKeyFromScript(&m_nIp);
		//strncpy(strbuff, str, KEY_LENGTH_IN_SCRIPT);

		CollectParameters(&m_nIp, 14); //---------------------TODO vcs 14 re this block but uses 12 todo?
		...
		int zone = CTheZones::FindZoneByLabelAndReturnIndex((char*)str, ZONE_INFO);
		if (zone < 0) {
		...
		while (zone >= 0) {
			...
			zone = CTheZones::FindNextZoneByLabelAndReturnIndex((char*)str, ZONE_INFO);
		}
		return 0;
	}
	case COMMAND_REQUEST_ANIMATION:
	{
		...
		const char *label = GetKeyFromScript(&m_nIp);
		CStreaming::RequestAnim(CAnimManager::GetAnimationBlockIndex(label), (m_bIsMissionScript ? STREAMFLAGS_SCRIPTOWNED : STREAMFLAGS_AMBIENT_SCRIPT_OWNED));
		return 0;
	}
	...
	{
		//script_assert(false && "VCS NEED TECT COMM: COMMAND_DISPLAY_NTH_ONSCREEN_COUNTER_WITH_STRING");
		//char onscreen_str[12];
		//script_assert(CTheScripts::ScriptSpace[m_nIp++] == ARGUMENT_GLOBALVAR);
		uint16 var = (uint8*)GetPointerToScriptVariable(&m_nIp) - CTheScripts::ScriptSpace;
		CollectParameters(&m_nIp, 2);
		uint32 type = GET_INTEGER_PARAM(0);
		uint32 pos = GET_INTEGER_PARAM(1);
		//wchar* text = TheText.Get((char*)&CTheScripts::ScriptSpace[m_nIp]); // ???
		//strncpy(onscreen_str, (char*)&CTheScripts::ScriptSpace[m_nIp], KEY_LENGTH_IN_SCRIPT);
		//m_nIp += KEY_LENGTH_IN_SCRIPT;
		const char *label = GetKeyFromScript(&m_nIp); // vcs
		CUserDisplay::OnscnTimer.AddCounter(var, type, (char *)label, pos - 1, -1, nil, 0);
		return 0;
	}
	...
	{
		script_assert(false && "VCSTODO CHECK COMM: COMMAND_IS_PLAYER_WEARING");
		CollectParameters(&m_nIp, 1);
		// lcs
		//char key[12];
		//CTheScripts::ReadTextLabelFromScript(&m_nIp, key);
		...
		script_assert(pPed);
		UpdateCompareFlag(CModelInfo::GetModelInfo(pPed->GetModelIndex())->GetNameHashKey() == CKeyGen::GetUppercaseKey(strbuff));
		return 0;
	}
	...
	case COMMAND_CLOSE_ALL_CAR_DOORS:
	{
		...
		CAutomobile* pCar = (CAutomobile*)CPools::GetVehiclePool()->GetAt(GET_INTEGER_PARAM(0));
		script_assert(pCar&& pCar->IsCar());
		debug("VCS TODO: SOME CODE IN COMM: COMMAND_CLOSE_ALL_CAR_DOORS\n"); // if else and calls
		pCar->CloseAllDoors();
		return 0;
	}
