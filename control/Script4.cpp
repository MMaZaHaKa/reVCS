...
int8 CRunningScript::ProcessCommands800To899(int32 command)
{
	...
	case COMMAND_SET_PLAYER_DRUNKENNESS:
	{
		...
		GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0)); // vcs
	...
	case COMMAND_CHECK_FOR_PED_MODEL_AROUND_PLAYER:
	{
		...
		// vcs (ida style)  NEED TEST THIS!! VCS
		CPed *pPlayerPed = CPools::GetPedPool()->GetAt(GET_INTEGER_PARAM(0));
		script_assert(pPlayerPed);
		CVector d1 = pPlayerPed->GetPosition() - GET_VECTOR_PARAM(1);
		CVector d2 = pPlayerPed->GetPosition() + GET_VECTOR_PARAM(1);
		// or
		//GetPlayerInfoByPoolHandler(pPlayerInfo, GET_INTEGER_PARAM(0));
		//CVector d1 = pPlayerInfo->GetPos() - GET_VECTOR_PARAM(1);
		//CVector d2 = pPlayerInfo->GetPos() + GET_VECTOR_PARAM(1);
	...
	case COMMAND_LOAD_MISSION_TEXT:
	{
		...
		TheText.LoadMissionText((char*)key);
		return 0;
	}
	...
	case COMMAND_IS_PLAYER_IN_INFO_ZONE:
	{
		...
		char strbuff[16];
		memset(strbuff, 0, KEY_LENGTH_IN_SCRIPT); // :/
		...
		UpdateCompareFlag(strncmp(strbuff, infoZone->name, 8) == 0); // original code doesn't seem to be using strncmp in here and compare 2 ints instead
		return 0;
	}
	...
	case COMMAND_IS_CAR_AUTOMOBILE:
	{
		...
		CVehicle* pVehicle = CPools::GetVehiclePool()->GetAt(GET_INTEGER_PARAM(0));
		script_assert(pVehicle);
		UpdateCompareFlag(pVehicle->GetModelIndex() >= MI_FIRST_VEHICLE && pVehicle->GetModelIndex() < MI_FERRY); // 148? < vcs =< 170
		return 0;
	}
	case COMMAND_IS_CAR_BIKE:
	{
		...
		CVehicle* pVehicle = CPools::GetVehiclePool()->GetAt(GET_INTEGER_PARAM(0));
		script_assert(pVehicle);
		UpdateCompareFlag(pVehicle->GetModelIndex() >= MI_ANGEL && pVehicle->GetModelIndex() <= MI_SANCHEZ); // vcs ^6 ?
		return 0;
	}
		return 0;
	case COMMAND_IS_CAR_PLANE:
	{
		...
		CVehicle* pVehicle = CPools::GetVehiclePool()->GetAt(GET_INTEGER_PARAM(0));
		script_assert(pVehicle);
		UpdateCompareFlag(pVehicle->GetModelIndex() >= MI_AIRTRAIN && pVehicle->GetModelIndex() <= MI_DEADDODO); // vcs ^5 ?
		return 0;
	}
	case COMMAND_IS_CAR_HELI:
	{
		...
		CVehicle* pVehicle = CPools::GetVehiclePool()->GetAt(GET_INTEGER_PARAM(0));
		script_assert(pVehicle);
		UpdateCompareFlag(pVehicle->GetModelIndex() >= MI_ESCAPE && pVehicle->GetModelIndex() <= MI_CHOPPER); // vcs ^4 ?
		return 0;
	}
...
int8 CRunningScript::ProcessCommands900To999(int32 command)
{
	...
	case EMPTY_COMMAND_912: // they DO have it in script
		return 0;
	...
	case EMPTY_COMMAND_925:
	{
		/* Something must have been here */
		return 0;
	}
	case EMPTY_COMMAND_926:
	{
		/* Something must have been here */
		return 0;
	}
	case EMPTY_COMMAND_927:
	{
		/* Something must have been here */
		return 0;
	}
	case COMMAND_SET_DEBUG_MENU_ACTIVE:
		CollectParameters(&m_nIp, 1);
		// this sets two values on PS2 and PSP, but not on mobile - TODO? // mazahaka vcs empty ps2
		return 0;
	...
	case COMMAND_SET_UNIQUE_JUMPS_FOUND:
		...
		return 0;
	case EMPTY_COMMAND_941: // like COMMAND_LINE
	{
		CollectParameters(&m_nIp, 1);
		/* Something must have been here */
		return 0;
	...
	case COMMAND_GET_MULTIPLAYER_MODE:
		SET_INTEGER_PARAM(0, 0); // TODO: VCS EMPTY
		StoreParameters(&m_nIp, 1);
		return 0;
	...
	case EMPTY_COMMAND_995:
	{
		/* Something must have been here */
		return 0;
	}
	case EMPTY_COMMAND_996:
	{
		/* Something must have been here */
		return 0;
	}
